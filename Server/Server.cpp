...
std::string function(const std::string buffer)
{
  ...
  // Регулярное выражение для выражений вида "число оператор число оператор число".
  std::regex pattern_3nums_none(R"(^\s*([0-9]+(?:\.[0-9]+)?)\s*([\+\-\*\/])\s*([0-9]+(?:\.[0-9]+)?)\s*([\+\-\*\/])\s*([0-9]+(?:\.[0-9]+)?)\s*$)");
  ...
  {
    // Обрабатываем выражение вида "число оператор число".
    try
    {
  ...
  else if (std::regex_match(temp, matches, pattern_3nums_right))
  {
    // Обрабатываем выражение вида "число оператор (число оператор число)".
    try
    {
  ...
  else if (std::regex_match(temp, matches, pattern_3nums_left))
  {
    // Обрабатываем выражение вида "(число оператор число) оператор число".
    try
    {
      ...
      {
      case '+':
        inner_result = addition(num1, num2); // Вычисляем сложение чисел во скобках.
        break;
      case '-':
        inner_result = deduction(num1, num2); // Вычисляем вычитание чисел во скобках.
        break;
      case '*':
        inner_result = multiplication(num1, num2); // Вычисляем умножение чисел во скобках.
        break;
      case '/':
        inner_result = separation(num1, num2); // Вычисляем деление чисел во скобках.
        break;
      default:
        // Если оператор не распознан, устанавливаем сообщение об ошибке и завершаем функцию
        std::cerr << "Неверная опif(resul)ерация во скобках на сервере\n"; // Выводим сообщение об ошибке в поток cerr
      ...
      {
      case '+':
        result = addition(inner_result, num3); // Вычисляем сложение.
        break;
      case '-':
        result = deduction(inner_result, num3); // Вычисляем вычитание.
        break;
      case '*':
        result = multiplication(inner_result, num3); // Вычисляем умножение.
        break;
      case '/':
        result = separation(inner_result, num3); // Вычисляем деление.
        break;
      default:
        std::cerr << "Неверная внешняя операция на сервере\n"; // Выводим сообщение об ошибке в поток cerr
        return "Неверная внешняя операция на сервере.";        // Копируем сообщение об ошибке в буфер
  ...
  else if (std::regex_match(temp, matches, pattern_3nums_none))
  { // Обрабатываем выражение вида "число оператор число оператор число".
    try
    {
